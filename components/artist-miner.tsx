    ...
    <div className="space-y-4">
        ...
        {mockArtists.slice(0, 4).map((a) => (
          <button key={a.id} onClick={() => setSelectedArtistId(a.id)}
            className={cn("rounded-full px-3 py-1.5 text-xs font-medium transition",
              selectedArtistId === a.id ? "bg-cyan-500 text-white shadow-sm shadow-cyan-500/30" : "border border-slate-200 bg-white text-slate-600 hover:border-slate-300 hover:text-slate-800"
            )}>{a.name}</button>
        ))}
      </div>
          ...
          <div className="flex items-start justify-between gap-4">
            <div>
              ...
              <div className="mt-2 flex flex-wrap gap-1">
                {artist.genres.map((g) => <span key={g} className="rounded-full bg-slate-100 px-2 py-0.5 text-[10px] text-slate-500">{g}</span>)}
              </div>
            </div>
      ...
      <div className="flex items-center justify-between">
        ...
        <button onClick={() => setSortDesc((d) => !d)}
          className="flex items-center gap-1.5 rounded-lg border border-slate-200 bg-white px-3 py-1.5 text-xs text-slate-500 shadow-sm transition hover:bg-slate-50 hover:text-slate-700">
              ...
              {entries.map((entry) => {
                ...
                return (
                  <tr key={entry.id} className="group transition hover:bg-slate-50/60">
                    <td className="w-14 px-4 py-3 font-mono text-xs text-slate-400">{entry.year}</td>
                    ...
                    <td className="px-4 py-3 text-xs text-slate-500">{entry.label}</td>
                    <td className="px-4 py-3 font-mono text-xs text-slate-400">{entry.catalogNumber ?? "—"}</td>
                    <td className="px-4 py-3 font-mono text-xs text-slate-500">{entry.bpm ?? "—"}</td>
                    <td className="px-4 py-3">
                    ...
                    <td className="px-4 py-3">
                      <div className="flex items-center justify-end gap-1.5 opacity-0 transition group-hover:opacity-100">
                        ...
                        <button className="flex h-6 w-6 items-center justify-center rounded text-slate-300 hover:bg-slate-100 hover:text-slate-500"><ExternalLink className="h-3 w-3" /></button>
                      </div>
                    </td>
                  </tr>
                );
