...
const PHASES: { key: SetPhase; label: string; color: string; gradient: string }[] = [
  { key: "warmup", label: "Warmup", color: "text-blue-400", gradient: "from-blue-500/20 to-blue-500/5" },
  { key: "groove", label: "Groove", color: "text-teal-400", gradient: "from-teal-500/20 to-teal-500/5" },
  { key: "build", label: "Build", color: "text-yellow-400", gradient: "from-yellow-500/20 to-yellow-500/5" },
  { key: "peak", label: "Peak", color: "text-orange-400", gradient: "from-orange-500/20 to-orange-500/5" },
  { key: "close", label: "Close", color: "text-purple-400", gradient: "from-purple-500/20 to-purple-500/5" },
];
    ...
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between">
        ...
        <button
          ...
          disabled={building}
          className="flex items-center gap-2 rounded-lg bg-teal-500/20 px-4 py-2 text-sm font-medium text-teal-300 ring-1 ring-teal-500/30 transition hover:bg-teal-500/30 disabled:opacity-50"
        >
      ...
      <div className="rounded-xl border border-white/8 bg-[#15151B] p-4">
        ...
        <div className="flex items-end gap-1 h-16">
          {tracks.map((t) => (
            <div
              key={t.id}
              className="flex-1 rounded-t-sm bg-gradient-to-t from-teal-600/80 to-teal-400/50 transition-all"
        ...
        {PHASES.map((phase) => {
          ...
          return (
            <div
              key={phase.key}
              className={cn(
                "rounded-xl border border-white/8 bg-gradient-to-b p-3",
                phase.gradient
              )}
            ...
            >
              ...
              <div className="mb-3 border-b border-white/8 pb-2">
                ...
                <p className="text-[10px] text-zinc-600">{phaseTracks.length} tracks</p>
              </div>
              ...
              <div className="space-y-2">
                {phaseTracks.map((track) => (
                  <div
                    ...
                    className={cn(
                      "group cursor-grab rounded-lg border border-white/8 bg-[#0E0E10]/60 p-2.5 transition active:cursor-grabbing",
                  ...
                  >
                    <div className="flex items-start gap-1.5">
                      ...
                      <div className="min-w-0">
                        <p className="truncate text-xs font-medium text-zinc-200">{track.artist}</p>
                        <p className="truncate text-[10px] text-zinc-500">{track.title}</p>
                        <div className="mt-1.5 flex items-center gap-2">
                          <span className="flex items-center gap-0.5 font-mono text-[9px] text-zinc-600">
                ...
                {phaseTracks.length === 0 && (
                  <div className="flex h-16 items-center justify-center rounded-lg border border-dashed border-white/8 text-[10px] text-zinc-700">
                    Drop tracks here
                  </div>
                )}
              </div>
      ...
      <div className="grid grid-cols-3 gap-3">
        {[
          ...
          <div key={label} className="flex items-center gap-3 rounded-xl border border-white/8 bg-[#15151B] p-3">
